...
import urllib
import urllib2
API_TOKEN = "635554915:AAFVe6nAU66LYVHSZVFGakqiA2rGxwxu0tc"
...
def admin(user_id):
    Admins = [1, 449967898] #Adminlar id si ro'yhati. Bu yerga o'zingizni id raqamingizni yozing. Tel raqam emas, telegramdagi id raqam
    ...
    if len(text)>0: #agar text uzunligi 0 dan kotta bo'sa (hatolarni oldini olish uchun
        ...
        def start():
            if getEnabled(chat_id): #agar oldin yozgan bo'sa
                bot.send_message(chat_id, "Assalomu aleykum, qalesiz?")
